...
`@qudtlib/allunits` ships only English (`en` / `en-US`) labels. This package patches `Unit` and `QuantityKind` instances at import time with ~15,000 labels from three sources:

- **[QUDT ontology](https://qudt.org/vocab/unit)** — 20+ languages for units and quantity kinds
- **[Wikidata](https://www.wikidata.org/)** — French (`fr`) and German (`de`), filling gaps not covered by QUDT
- **AI/manual translation** — French (`fr`) and German (`de`), for the ~700 quantity kinds not covered by either of the above
...
> **Note on German translations:** German also has 100% quantity kind coverage, using the same AI-translation fallback approach. Unlike the French labels, the German ones have **not yet been reviewed** by a German speaker — treat them as unverified until reviewed.
...
| Wikidata | French (`fr`), German (`de`) — units and quantity kinds only |
| AI translation *(unofficial)* | French (`fr`), German (`de`, unreviewed) — quantity kinds only, where QUDT/Wikidata have no label in that language |
